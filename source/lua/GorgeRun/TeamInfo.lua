...
function TeamInfo:UpdateInfo()
    ...
    if self.team then
        ...
        for index, player in ipairs(self.team:GetPlayers()) do
            ...
            --------------------------------------------------------------------
            if player:GetResources() == 0 or RoundStartTimeOffset[player] == nil then -- Gorge just joined OR DIED so (re)set start time
                RoundStartTimeOffset[player] = Shared.GetTime()
                totaltime = .1
            else
                totaltime = Shared.GetTime() - RoundStartTimeOffset[player]
                if totaltime < 1 then
                totaltime = .1
                end
            end
            player:SetResources(totaltime)
